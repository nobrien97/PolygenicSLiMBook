...
# 6.7 Writing SLiM code with parallelism in mind
...
there is no chance of the output being overwritten[^fn3]. At the end of your simulations, you can join all these files into one using the 
```cat``` bash command:
...
```
Which will apend a newline character to the end of each file[^fn4].
...
[^fn3]: There is still a small chance of ```stdout``` losing your output if you have a massive amount of I/O that is overloading the filesystem. But just don't do that.
[^fn4]: https://stackoverflow.com/a/31053205/13586824